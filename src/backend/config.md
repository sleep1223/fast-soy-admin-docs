...
| `RADAR_ENABLED` | `true` | 启用 fastapi-radar（请求/SQL/异常 dashboard） |
| `GUARD_ENABLED` | `true` | 启用 fastapi-guard 限流 |
...
模块独立 DB 行为详见 [自动发现 / 业务模块独立数据库](./core/autodiscover.md#业务模块独立数据库)。
...
详细命令见 [命令参考](./commands.md) 与 [规范 / 后端](../standard/backend.md)。
...
详见 [Sqids / 轮换 SECRET_KEY 怎么办](./core/sqids.md#轮换-secret_key-怎么办)。
