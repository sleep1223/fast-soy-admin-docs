...
系统模块的核心模型在 [`app/system/models/admin.py`](../../../app/system/models/admin.py) 与 [`app/system/models/dictionary.py`](../../../app/system/models/dictionary.py)。所有模型都继承 `BaseModel + AuditMixin`，外键 / 主键在 HTTP 边界自动编码为 [sqid](./core/sqids.md)。
...
- 通过 `.env` 的 `DB_URL` 切换到 PostgreSQL / MySQL / SQL Server（**无需改代码**），详见 [切换数据库](./database.md)
