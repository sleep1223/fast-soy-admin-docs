...
当接口只被一个人、一秒一次地调用时，什么并发控制都不需要；但后台管理系统里总有几种操作并不满足这个假设：
...
[状态机](./core/state-machine.md) 只校验 `from → to` 合法，**不防止两个请求同时把 `pending` 改成 `active`**。要做到"同时只有一次流转生效"，搭配**乐观锁**。
...
这个模式需要配一个**对账任务**（cron）扫 `status="pending"` 且超时的记录，主动去查外部真实状态。正是[事件总线](./core/events.md)里强调的"先写 DB 再发副作用"的延伸。
...
- [CRUDBase](./crud.md) — `CRUDBase.update` 内部已经带事务
- [状态机](./core/state-machine.md) — FSM 只管合法性，想防并发请叠乐观锁
- [事件总线](./core/events.md) — 事务提交后再发事件的推荐姿势
