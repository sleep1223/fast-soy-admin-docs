...
System models live in `app/system/models/admin.py` and `app/system/models/dictionary.py`. All inherit `BaseModel + AuditMixin`; primary keys and foreign keys are auto-encoded as [sqid](/en/backend/core/sqids) at the HTTP boundary.

> Business module models live in `app/business/<name>/models.py`. See [Mixins](/en/backend/mixins) and [HR module](/en/backend/business/hr) for conventions.
...
> `token_version` is the JWT invalidation primitive: `INCR` once after password change / forced logout / impersonation exit and all old tokens fail. See [Auth](/en/backend/auth#token-invalidation-token_version).
...
| `data_scope` | enum(`DataScopeType`) default=`all` | row-level scope (see [data scope](/en/backend/data-scope)) |
...
Developers **never** maintain the `Api` table by hand. See [RBAC / API auto-reconcile](/en/backend/rbac#api-auto-reconcile).
...
Naming convention: `B_<MODULE>_<RESOURCE>_<ACTION>`. See [RBAC / button naming](/en/backend/rbac#button-naming-convention).
...
- Switch to PostgreSQL / MySQL / SQL Server via `.env` `DB_URL` (**no code change**), see [Switching DB](/en/backend/database)
...
- [Mixins](/en/backend/mixins) — `BaseModel / AuditMixin / TreeMixin / SoftDeleteMixin`
- [Sqids](/en/backend/core/sqids) — how PK / FK become sqid strings
- [RBAC](/en/backend/rbac) — User / Role / Menu / Button / API together
- [HR module](/en/backend/business/hr) — business module model sample
