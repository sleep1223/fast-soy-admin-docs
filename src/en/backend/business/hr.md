...
> Once `reconcile_menu_subtree` is enabled, **the subtree is treated as IaC** — Web-UI-created menus / buttons under it are reaped on next restart. See [Init data](/en/backend/init-data).
...
- [Init data](/en/backend/init-data) — `ensure_menu / ensure_role / reconcile_menu_subtree` semantics
- [Auth](/en/backend/auth) — JWT, dependencies, button permissions
- [CRUDBase](/en/backend/crud) — `CRUDBase / build_search / SearchFieldConfig`
- [API conventions](/en/backend/api) — paths / methods / response format
