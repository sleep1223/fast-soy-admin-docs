...
just logs                   # 等价的 justfile 入口；可用 `just logs app` 过滤服务，`just logs app 200` 指定行数
```
...
```bash
git pull
docker compose down && docker compose up -d --build  # 代码变更要加 --build（或 just rebuild）
...
`just up` / `docker compose up -d` **不会自动建表**。新库第一次启动时，后端连上去会发现表不存在（日志里能看到 `no such table: ...` 之类的报错）——这是预期行为，手动触发一次 `initdb` 即可：
...
# --- 本地（开发机） ---
# 1. 改 app/**/models.py
just makemigrations                              # 生成 migrations/models/*.py
